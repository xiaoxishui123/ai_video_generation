...
> **作者**: xiaoxishui  
> **更新日期**: 2025-12-04
> **变更说明**: 修复火山方舟图生视频内网图片URL无法访问问题，自动转换为Base64格式
...
| 平台 | 模型ID | 功能 | 说明 |
|------|--------|------|------|
| 阿里云百炼 | `wan2.5-t2v-preview` | 文生视频 | 通义万相 T2V (最新) |
...
| 内容违规 | 提示词不合规 | 修改提示词内容 |
| image_url: image not found | 内网图片URL | v0.0.6已自动转换为Base64 |

### 图片URL处理说明

**火山方舟**平台的API服务器位于北京机房，无法访问内网地址。插件会自动检测以下情况并转换为Base64格式：

- ✅ 私有IP地址（10.x.x.x, 172.16-31.x.x, 192.168.x.x）
- ✅ localhost / 127.0.0.1
- ✅ 非标准端口（如 8080、3000 等）
- ✅ Dify内部文件URL（如 `http://x.x.x.x:8080/files/...`）

**支持的公网URL**：
- 阿里云OSS（oss-cn-*.aliyuncs.com）
- 腾讯云COS（*.myqcloud.com）
- 火山引擎TOS（tos-cn-*.volces.com）
- 其他标准HTTP/HTTPS公网地址
